...
function parseMarkdupMetrics(file) {
  ...
  if (!header || !values || header.length !== values.length) {
    return {};
  }
...
function valueForChart(row, key) {
  const value = row[key];
...
function renderBars(rows, key, label) {
  const chartRows = rows
    .map((row) => ({ sample: row.sample_id, value: valueForChart(row, key) }))
    .filter((row) => row.value !== null);
  const max = Math.max(...chartRows.map((row) => row.value), 0);

  if (chartRows.length === 0) {
    return `<p class="empty">No data for ${escapeHtml(label)}.</p>`;
  }
...
const metaMap = new Map(sampleMetaFiles.map((file) => {
  const meta = parseKeyValueFile(file);
  return [meta.sample_id, meta];
}));

const manifestSamples = readManifestSamples(path.join(OUT_DIR, "run.manifest.txt"));
const sampleIds = manifestSamples.length > 0 ? manifestSamples : [...metaMap.keys()].sort();
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .hero {
      padding: 24px;
      margin-bottom: 18px;
      background: linear-gradient(135deg, rgba(154,52,18,.98), rgba(120,53,15,.92));
      color: white;
    }
...
<body>
  <div class="wrap">
    <section class="hero">
      <h1>WES QC Summary</h1>
      <p>Current staged output summary for fastp and MarkDuplicates derived metrics.</p>
    </section>
    <section class="grid">
      <div class="card"><div class="label">Samples</div><div class="value">${summary.sample_count}</div></div>
      <div class="card"><div class="label">Families</div><div class="value">${summary.family_count}</div></div>
      <div class="card"><div class="label">Affected</div><div class="value">${summary.affected_count}</div></div>
      <div class="card"><div class="label">With gVCF</div><div class="value">${summary.samples_with_gvcf}</div></div>
      <div class="card"><div class="label">With fastp QC</div><div class="value">${summary.samples_with_fastp}</div></div>
      <div class="card"><div class="label">With duplication metrics</div><div class="value">${summary.samples_with_markdup_metrics}</div></div>
    </section>
    <section class="layout">
      <div class="panel">
        <h2>fastp retained reads (%)</h2>
        ${renderBars(rows, "fastp_retained_read_pct", "fastp_retained_read_pct")}
      </div>
      <div class="panel">
        <h2>fastp Q30 (%)</h2>
        ${renderBars(rows, "fastp_q30_rate", "fastp_q30_rate")}
      </div>
      <div class="panel">
        <h2>GC content (%)</h2>
        ${renderBars(rows, "fastp_gc_content", "fastp_gc_content")}
      </div>
      <div class="panel">
        <h2>Duplication (%)</h2>
        ${renderBars(rows, "percent_duplication", "percent_duplication")}
      </div>
    </section>
    <section class="panel">
